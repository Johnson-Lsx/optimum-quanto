...
__all__ = ["QModuleMixin", "QAttentionModuleMixin", "register_qmodule", "quantize_module"]
...
class QModuleMixin(ABC):
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        ...
        destination[prefix + "input_scale"] = (
            self.input_scale if keep_vars else self.input_scale.detach()
        )
        destination[prefix + "output_scale"] = (
            self.output_scale if keep_vars else self.output_scale.detach()
        )
        destination[prefix + "weight_qtype"] = (
            "none" if self.weight_qtype is None else self.weight_qtype.name
        )
    ...
    @classmethod
    def qcreate(
        cls, module: torch.nn.Module, weights: Optional[qtype], activations: Optional[qtype] = None
    ):
        ...
        if self.activation_qtype is not None:
            if isinstance(output, QBytesTensor):
                ...
                output = quantize_activation(
                    output, qtype=self.activation_qtype, scale=self.output_scale
                )
        return output
...
class QAttentionModuleMixin(ABC):
    def __init__(
        self,
        *args,
        weights: Optional[Union[qtype, str]] = None,
        ...
        **kwargs,
        ...
        if mro.index(__class__) > mro.index(torch.nn.Module):
            raise TypeError(
                "QAttentionModuleMixin must be placed before any torch.nn.Module class in quantized module inheritance."
            )
        # This will setup the torch.nn.Module
        super().__init__(*args, **kwargs)
        if weights is not None and not isinstance(weights, qtype):
    ...
    def _save_to_state_dict(self, destination, prefix, keep_vars):
        if self.weight_qtype is None or not self.frozen:
            # Save standard weight Tensor
            destination[prefix + "in_proj_weight"] = (
                self.in_proj_weight if keep_vars else self.in_proj_weight.detach()
            )
        else:
            # Save QTensor using dedicated method
            self.in_proj_weight.save_to_state_dict(
                destination, prefix + "in_proj_weight.", keep_vars
            )
        if self.in_proj_bias is not None:
            destination[prefix + "in_proj_bias"] = (
                self.in_proj_bias if keep_vars else self.in_proj_bias.detach()
            )
        destination[prefix + "input_scale"] = (
            self.input_scale if keep_vars else self.input_scale.detach()
        )
    ...
    def _load_from_state_dict(
        ...
        weight_name = prefix + "in_proj_weight"
        if self.weight_qtype is not None and weight_name not in state_dict:
            # The weight Tensor is not present because it is a flattened QTensor
            weight_prefix = weight_name + "."
            if self.weight_qtype.bits == 8:
            ...
            assign_to_params_buffers = local_metadata.get("assign_to_params_buffers", False)
            if assign_to_params_buffers:
                self.in_proj_weight = torch.nn.Parameter(deserialized_weight)
            else:
                if type(self.in_proj_weight.data) != type(deserialized_weight):
                    # Reloading frozen weights into unfrozen module: move to the correct device and force assignment
                    self.in_proj_weight = torch.nn.Parameter(
                        deserialized_weight.to(self.in_proj_weight.device)
                    )
                else:
                    # FIXME: here we should copy frozen weights into frozen module, but this leads to grad error
                    self.in_proj_weight = torch.nn.Parameter(
    ...
    def from_module(
        ...
        if qmodule is None:
            return None
        with torch.no_grad():
            qmodule.in_proj_weight.copy_(module.in_proj_weight)
            if module.in_proj_bias is not None:
                qmodule.in_proj_bias.copy_(module.in_proj_bias)
        return qmodule.to(module.in_proj_weight.device)
    ...
    def qweight(self):
        ...
        if self.weight_qtype is None:
            # QModule that does not quantize its weights
            return None
        if isinstance(self.in_proj_weight, QTensor):
            # Frozen QModule
            return self.in_proj_weight
        # Quantize dynamically the weights per-axis
        quantized_in_proj_weight = quantize_weight(
            self.in_proj_weight,
            qtype=self.weight_qtype,
            axis=0,
            group_size=self.weight_group_size,
        ...
        return quantized_in_proj_weight
    ...
    def freeze(self):
        qin_proj_weight = self.qweight
        if qin_proj_weight is not None:
            # Replace float weights by quantized weights
            self.in_proj_weight = torch.nn.Parameter(qin_proj_weight)

    @property
    def frozen(self):
        return isinstance(self.in_proj_weight, QTensor)
